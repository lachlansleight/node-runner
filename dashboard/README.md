...
- Create app form with a **GitHub repo picker** (when the GitHub App is connected): pick a
  repo + branch and pushes auto-deploy. Falls back to a manual git URL otherwise.
...
| Var | Notes |
|-----|-------|
| `AGENT_URL` | your control domain, e.g. `https://control.<yourdomain>` |
...
To enable the repo picker and push-to-deploy, connect a GitHub App on the box — see
